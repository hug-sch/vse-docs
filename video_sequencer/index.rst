...
Blender is a versatile software: modeling, sculpting, 2D drawing, animation, video editing. Most of the work is done in so-called editors: tools for viewing and modifying your work through a specific part of Blender.
...
It is not always obvious whether to describe something in the Video Sequencer section or in the Video Editing section. We try to follow the following rule: Anything that comes down to how to move around the editor(view menu, overlays, display modes, ...) is described in the Editor section. Anything that involves working with the editors data (e.g. strips in case of the VSE) will go into the video editing section. For example, selecting strips is described in the Video Editing section, while zooming in on the timeline is covered in the Editor section.

.. figure:: /images/editors_vse_sequencer.svg
   :alt: Main view of Sequencer
   
   Figure 1: Sequence Editor main view: Header (red), Preview (Yellow), Sequencer (blue), Properties (Grey), Panels & Tabs (Yellow), Toolbar (Purple)

The VSE can contain various items. They are described into more detail in the next sections. Figure 1 shows a combined view *Sequencer/Preview*. You can select this view with the View Type selector. This view can contain the following areas:
...
With the View Type selector you can choose if the Video Sequence Editor contains only the Sequencer or only the Preview or both (see figure 2).
